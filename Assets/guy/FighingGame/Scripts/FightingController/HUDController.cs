...
public class HUDController : MonoBehaviour
{
    ...
    [Header("UI To Hide At Game End")]
    [Tooltip("ลากตัวแม่ของแถบเลือด/เวลา มาใส่ตรงนี้ เพื่อให้มันหายไปตอนจบเกม")]
    public GameObject[] hudsToHideAtEnd;
    ...
    public void HideGameHUDs()
    {
        // 1. ซ่อนตามที่ลากมาใส่ใน Inspector (ถ้ามี)
        if (hudsToHideAtEnd != null)
        {
            foreach (var go in hudsToHideAtEnd)
            {
                if (go != null) go.SetActive(false);
            }
        }

        // 2. ค้นหาแบบครอบจักรวาลทั้งเกม (GameObject.Find) 
        // ทำงานควบคู่กันไปเลย ป้องกันกรณีที่คุณลืมลากใส่ หรือลากใส่ผิด
        GameObject autoGeneratedRoot = GameObject.Find("TekkenHealthBars_Generated");

        // ถ้าเจอตัวที่สร้างอัตโนมัติ ให้ปิดทิ้งทั้งก้อนเลย 
        if (autoGeneratedRoot != null)
        {
            autoGeneratedRoot.SetActive(false);
        }
        else
        {
            // เซฟตี้: ถ้าไม่ได้สร้างอัตโนมัติ ก็พยายามปิดตัวแม่ของหลอดเลือด
            if (playerHealthFill != null && playerHealthFill.transform.parent != null) 
                playerHealthFill.transform.parent.gameObject.SetActive(false);
                
            if (opponentHealthFill != null && opponentHealthFill.transform.parent != null) 
                opponentHealthFill.transform.parent.gameObject.SetActive(false);
        }
            
        // ปิดเวลา และ ตัวหนังสือคอมโบที่สร้างอัตโนมัติด้วย
        if (timerText != null) 
            timerText.gameObject.SetActive(false);
        if (playerComboText != null) 
            playerComboText.gameObject.SetActive(false);
        if (opponentComboText != null) 
            opponentComboText.gameObject.SetActive(false);
    }
}
